...
async function main() {
  const sheetPath = join(
    process.cwd(),
    "scripts",
    "c5-blind-labelling-sheet.json",
  );
  const expectationsPath = join(
    process.cwd(),
    "scripts",
    "retrieval-expectations.json",
  );

  const sheet = JSON.parse(await readFile(sheetPath, "utf8")) as {
    pairs: Pair[];
  };
  ...
  console.log(
    `Overall agreement: ${agree}/${total} = ${overallAgreement.toFixed(3)}`,
  );
  console.log();
  ...
  for (const c of classes) {
    console.log(
      `  ${c.padEnd(20)} ${String(confusion[c].should_match).padStart(8)}     ${String(confusion[c].partial).padStart(6)}  ${String(confusion[c].should_not_match).padStart(12)}`,
    );
  }
  ...
  const partialTotal =
    confusion.partial.should_match +
    confusion.partial.partial +
    confusion.partial.should_not_match;
  if (partialTotal > 0) {
    ...
    if (partialAgreement < 0.6) {
      console.log(
        "  ⚠ Partial agreement is poor — criterion 2 (acid test) is still OPEN.",
      );
    } else {
  ...
  if (disagreements.length > 0) {
    ...
    for (const d of disagreements) {
      console.log(
        `  #${d.id} ${d.queryId} rank ${d.rank}: Devin=${d.devin} Founder=${d.founder} — ${d.title}`,
      );
    }
  }
}
