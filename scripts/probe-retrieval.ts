...
async function main() {
  ...
  ): "title_only" | "specs_only" | "both" | "neither" {
  ...
  console.log(
    `Target products: ${querySet.products.dropped.length} dropped + ${querySet.products.retained.length} retained\n`,
  );
  ...
  for (const r of results) {
    ...
    const targetHandles = new Set(
      declaredTargets.map((tid) => targetById.get(tid)).filter(Boolean),
    );
  ...
  console.log("\nPer-query scores:");
  console.log(
    "Q    | Type       | Query (50 chars)                                    | Results | Targets | R@10 | R@50 | Best | Retr",
  );
  console.log(
    "-----|------------|-----------------------------------------------------|---------|---------|------|------|------|-----",
  );
  for (const s of queryScores) {
    ...
    console.log(
      `${s.queryId} | ${s.type.padEnd(10)} | ${q} | ${String(s.totalResults).padStart(7)} | ${String(s.targetHits.length).padStart(7)} | ${r10.padEnd(4)} | ${r50.padEnd(4)} | ${best.padEnd(4)} | ${retr}`,
    );
  }
  ...
  const droppedHits: Array<{
    queryId: string;
    targetId: string;
    rank: number;
    fieldPresence: string;
  }> = [];
  const retainedHits: Array<{
    queryId: string;
    targetId: string;
    rank: number;
    fieldPresence: string;
  ...
  for (const s of queryScores) {
    for (const h of s.targetHits) {
      if (h.population === "dropped")
        droppedHits.push({
          queryId: s.queryId,
          targetId: h.targetId,
          rank: h.rank,
          fieldPresence: h.fieldPresence,
        });
      else if (h.population === "retained")
        retainedHits.push({
          queryId: s.queryId,
          targetId: h.targetId,
          rank: h.rank,
  ...
  console.log(
    `Retained-relational products: ${retainedHits.length} hits across all queries`,
  );

  if (droppedHits.length > 0) {
    const droppedBestRanks = droppedHits
      .map((h) => h.rank)
      .sort((a, b) => a - b);
    ...
    console.log(
      `  Dropped median rank: ${droppedBestRanks[Math.floor(droppedBestRanks.length / 2)]}`,
    );
    console.log(
      `  Dropped field presence: ${droppedHits.map((h) => h.fieldPresence).join(", ")}`,
    );
  } else {
    console.log(
      "  ⚠ NO dropped products retrieved by any query — this is the commercial consequence",
    );
  }

  if (retainedHits.length > 0) {
    const retainedBestRanks = retainedHits
      .map((h) => h.rank)
      .sort((a, b) => a - b);
    ...
    console.log(
      `  Retained median rank: ${retainedBestRanks[Math.floor(retainedBestRanks.length / 2)]}`,
    );
    console.log(
      `  Retained field presence: ${retainedHits.map((h) => h.fieldPresence).join(", ")}`,
    );
  }
  ...
  const _c2 = true; // determined by C5 hand-labelling below
  const _c3 = true; // determined by miss classification below
  const c4 = droppedHits.length > 0 || retainedHits.length > 0; // populations separable

  console.log(`1. Loop closes end-to-end: ${c1 ? "YES" : "NO"}`);
  console.log(
    `2. C5 produces ≥1 partial verdict (field presence could not produce): PENDING C5`,
  );
  console.log(
    `3. ≥1 miss classifies into TDD §6.2 class other than unexplained: PENDING C5`,
  );
  console.log(
    `4. Two populations separable and reported separately: ${c4 ? "YES" : "NO"}`,
  );
  ...
  for (const s of queryScores) {
    md.push(
      `| ${s.queryId} | ${s.type} | ${s.query} | ${s.totalResults} | ${s.targetHits.length} | ${s.recall10 === null ? "n/a" : s.recall10.toFixed(2)} | ${s.recall50 === null ? "n/a" : s.recall50.toFixed(2)} | ${s.bestRank ?? "—"} | ${s.retrievalRate === null ? "n/a" : s.retrievalRate} |`,
    );
  }
  md.push("");
  ...
  for (const h of droppedHits) {
    md.push(
      `- ${h.queryId}: ${h.targetId} at rank ${h.rank} (${h.fieldPresence})`,
    );
  }
  md.push("");
