...
const CatalogProduct = z
  .object({
    ...
    description: z
      .object({ plain: z.string().optional() })
      .passthrough()
      .optional(),
    metadata: z
      .object({
...
async function fetchStorefrontCatalogue(): Promise<StorefrontProductT[]> {
  ...
  for (let page = 1; page <= 8; page++) {
    ...
    if (!res.ok) {
      console.warn(
        `  storefront page ${page} → HTTP ${res.status}; stopping pagination`,
      );
      break;
    }
...
function sourceTextOf(sp: StorefrontProductT): string {
  const parts = [
    ...
    (sp.variants ?? [])
      .map((v) =>
        [v.title, v.sku, v.option1, v.option2, v.option3]
          .filter(Boolean)
          .join(" "),
      )
      .join(" "),
...
function salientTokens(claim: string): string[] {
  ...
  for (const m of claim.matchAll(/\b\d[\d.,]*\b/g))
    out.add(m[0].replace(/[.,]$/, ""));
...
async function main() {
  ...
  for (const cat of withSpecs.slice(0, CONFIG.sampleSize)) {
    ...
    if (!match || match.score < 0.3) {
      console.warn(
        `  ⚠ no confident storefront match for "${cat.title}" (best ${match?.score.toFixed(2)})`,
      );
      continue;
    }
  ...
  const flagged = rows.reduce(
    (n, r) => n + r.claims.filter((c) => c.flagged).length,
    0,
  );

  console.log(`\n${"═".repeat(64)}`);
  console.log(
    `${rows.length} product pairs · ${totalClaims} inferred claims · ${flagged} flagged for close reading`,
  );
  console.log("═".repeat(64));
  console.log(
...
async function emit(rows: any[], stats: Record<string, number>) {
  ...
  for (const [i, r] of rows.entries()) {
    ...
    md.push(
      `- **Source:** https://${CONFIG.shopDomain}/products/${r.storefrontHandle}`,
    );
    md.push(
      `- **Title match confidence:** ${r.matchScore.toFixed(2)} ${r.matchScore < 0.5 ? "⚠ verify this pairing before labelling" : ""}`,
    );
  ...
  await writeFile(
    jsonPath,
    JSON.stringify({ config: CONFIG, rows, stats, transcript }, null, 2),
    "utf8",
  );
