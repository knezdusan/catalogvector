...
| ID | Feature | Status | Notes |
|---|---|---|---|
| I-1 | Shopify Partner account | `PENDING` | Free, automatic approval |
| I-2 | Development store | `BLOCKED` | on I-1 |
| I-3 | Shopify AI Toolkit + Dev MCP in Claude Code | `PENDING` | Day-one; grounds the agent in live schemas |
| I-4 | UCP agent profile hosted at well-known URL | `PENDING` | **Lead-time risk — do first.** Trust tier governs rate limits and tool access |
