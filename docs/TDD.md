...
**Result — DIRECTIVE-8-v2 §8.4, executed 3 Aug 2026 at TSP:**

> **H4-R INCONCLUSIVE — the registered design cannot be executed at TSP.** 591 TSP products scraped. 35 title-absent (5.9% of catalog), 26 genuinely title-absent after manual verification. No single category has ≥6 title-absent AND ≥6 matched title-present controls of the same product type. The closest category is "shifter" (6 title-absent shift knobs, 10 title-present short shifters/cable bushings — different product types).

A best-available approximation was run with relaxed category matching (same store, different category). Result: title-absent 3/6 (0.500), title-present 1/6 (0.167), difference -0.333. This REJECTS by the rule, but the result is uninterpretable — the title-present controls were wrong product types for the queries (brake pads for spark plug queries, short shifters for shifter spring queries). The -0.333 is an artefact of category mismatch, not title coverage.

**Why the registered design cannot be executed:** TSP is a store with high title-coverage (94.1% of products have vehicles in titles). Title-absent products are concentrated in universal-fitment categories (shift knobs, universal fans, velocity stacks) where title-present controls of the same product type don't exist. MAP has the opposite problem — all brake pads use "Multiple Fitments" (no title-present controls). The replication is caught between two stores, neither of which can support the registered design.

H4 remains an auto-parts finding (TSP+MAP, 0.708 gap under relaxed matching), not a platform finding. H4-R does not disconfirm H4 — it cannot test it. Report: `docs/reports/directive8-stage4-followback.md`.
...
|---|---|---|---|
| Title-absent | 48 | 1 | 0.021 |
| Title-present | 48 | 0 | 0.000 |
| Difference | — | — | -0.021 |

Both populations scored near zero. Only 1 Intec product appeared across all 8 queries (1 in 400 slots, below chance). This is recorded as an **exploratory store-level observation** — the store itself is nearly invisible in the Catalog regardless of title. This is where its real value lies.
...
3. Store-level (Subimods, confirmed 3 Aug 2026): an enrolled store whose products don't rank for any query. Cause unknown. (Intec was initially classified as store-level but Stage 3 showed it is title-level — Intec products are findable by product-type queries, not vehicle-relational queries.)

### 6.1.9 Auto-parts re-sample — fitment-recall on 4 stores (DIRECTIVE-4 §4, executed 3 Aug 2026)

**Design.** Re-run the fitment-recall probe with P-1/P-2 hardening across 4 stores (TSP, MAP, Subimods, Springrates). 5 queries per store, store-stratified bucketing (15 thin + 15 rich), compliance threshold 15+ scored across 3+ stores (DIRECTIVE-4 §1.2).

**Result — 3 Aug 2026 (first compliant sample):**

> **COVERAGE GAP CONFIRMED** — corrected prefix=0.385, strict=0.313 < 0.80. 18 scored from 4 stores.

| Metric | Raw | Corrected |
|---|---|---|
| Scored products | 19 | 18 |
| Stores | 4 (TSP=10, MAP=4, Subimods=2, Springrates=3) | 4 (TSP=10, MAP=4, Subimods=1, Springrates=3) |
...
| 2026-08-03 | 0.8.1 | **DIRECTIVE-8-v2 Stage 1 — U-7 INCONCLUSIVE (unscoped floor exists). Page-boundary audit complete.** U-7 probes (5 probes, unscoped, depth 2000): U7-A (brake pads) 359, U7-B (running shoes) 256, U7-C (Paragon PBP370) 308, U7-D (nonsense "zxqv flurbin widget") 305 — UNSCOPED FLOOR, U7-E (progressive broadening): target absent for specific query, rank 23 for broader, rank 3 for brand+vehicle, rank 1 for brand only. Verdict: INCONCLUSIVE per pre-registered rule. Floor characterised: query-dependent (zero overlap between brake pads and nonsense), random products, absolutely invisible targets NOT in floor. Page-boundary audit: 17/18 final pages PARTIAL, all terminated with has_next_page=false, total_count matches actual for all 18. Report: `docs/reports/directive8-stage1-followback.md`. |
...
| 2026-08-06 | 1.1.0 | **DIRECTIVE-19 — instrument preparation, register correction, study pre-registration.** §3.1: Register entries 7, 9, 10 corrected (entry 7 superseded by entry 13, entry 9 corrupted U8-A figures replaced with D16 measurements, entry 10 restated with partition-based enumeration). Standing instruction added: every entry carries establishing + last-revising directive. Subimods handle count reconciled (13,358→13,107→13,257). §3.2: Attribution matcher built (brand + manufacturer part number from title + SKU). Coverage 75.4% (13,659/18,109), FPR 0.0%, FNR 0.0%. 60 same-part pairs, 50 near-miss pairs. I-6 satisfied. §3.3: MAP rarefaction curve — vendor/product_type saturate at ~70% of 25K metadata, log-log extrapolation to 102K estimates ~13,469 queries needed. Subimods selection error: 91% of absent products in cells with some recovery (within-cell problem, not partition coverage). §3.4: Same-day noise floor (running). §3.5: Store-selection rule pre-registered (independent frame, seed=42, size bands). §4.2: Design question — Allocation A (n=50 all) vertical CI 13.6pp, Allocation B (2×n=100+3×n=50) vertical CI 10.4pp. Between-store variation dominates. §5.1: Outreach scaled to 25 contacts, BRAND 3 rewritten, disclosure line added. §5.3: TDD §5 C9 and §8 amended — stores named in paper, notified before publication. §5.5: BLUEPRINT §5.1 updated — €500 tier removed, G3 reconciled at 25 contacts. |
